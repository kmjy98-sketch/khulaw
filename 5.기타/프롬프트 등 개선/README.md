...
> Codex 운영 메모(2026-06-15): 이 README는 Claude Code용 레거시 설명이다. 현재 워크스페이스에서는 `.agent/workflows/card-wiki-pipeline.md`를 우선하고, 이 패키지는 프롬프트·스크립트 참고용으로만 사용한다.

Claude Code 이관용 패키지. v3.5.2 수동 워크플로우(AI Studio·Gem·Claude Project 복사·붙여넣기)를 자동화.
...
실행:
  1. PDF 100p 단위 분할
  2. 각 청크 → Gemini 3.1 Pro로 OCR (Mode B default)
...
│   ├── 01_gemini3.1pro_마킹_구조_조문_추출_v3.6.md
...
│   ├── 01_ocr.py            (Gemini 3.1 Pro, v3.6 프롬프트 참조)
...
| 작업 | 비용 |
|---|---|
| 01 OCR (Gemini 3.1 Pro), 300p 책 | $1.5~3 |
| 02-wiki (Claude Opus 4.7), 300p 책 | $3~7 ⭐ 신규 단계 |
| 02-card (Claude Opus 4.7), 위키 출력 | $2~4 |
| 06b 사례집, 사례 20문제 | $1.5~3 |
| **책 1권 전체** | **$8~15** (v3.5.2 $5~10에서 약 1.8배 증가) |

위키화 단계가 추가되어 비용 증가하지만 책 1권당 5시간 절감.
월 한도 $50 default — 초과 시 자동 중단.

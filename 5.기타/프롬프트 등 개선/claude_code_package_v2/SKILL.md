# SKILL: 로스쿨 단권화·사례집 카드화 파이프라인 (v3.6 Phase 2 — 2모델 역할분담)

이 패키지는 한국 로스쿨 학습 자료(단권화 책·사례집)를 Obsidian 위키 마크다운으로 구조화 후 Anki 카드로 변환하는 **프롬프트 묶음**이다.
사용자(이권민)는 연세대 로스쿨 1학년, 변시 준비 중.

## v3.6 Phase 2 운영 모드 (2026-06-01)

**2모델 역할분담**: 안티그래비티(Gemini 3.5 Flash)가 OCR·위키화(본문 보존 구조화, 요약 아님)로 원문을 정리 → Claude Opus 4.8이 그 위키로 카드 빌드.

| 단계 | 담당 모델 | 운영 방식 |
|---|---|---|
| 01 OCR | 안티그래비티 앱(Gemini 3.5 Flash) | 원문 PDF 앱에 첨부 → `prompts/01_*` 적용 → 마킹·구조·조문 추출 (저비용 대용량) |
| 02-wiki | 안티그래비티 앱(Gemini 3.5 Flash) | 01 출력 → `prompts/02-wiki_*` 적용 → 위키 마크다운(본문 보존 구조화, 보존율 ≥ 0.90) |
| 02-card·06a·06b | Claude Opus 4.8 | 위키 입력 → `prompts/{단계}_claude_*.xml` 적용 → Anki TSV (최상급 추론) |
| 출력 | — | outputs/{단계}/에 저장 |
| 비용 논리 | — | Opus는 위키(구조화 텍스트)만 입력 → PDF 재OCR 불필요, 입력 비용↓ |

> Claude Code 단독 처리(이전 Phase 1.2)로 카드화 단계를 돌리는 것도 가능. Phase 2의 핵심 변화는 01 OCR·02-wiki를 안티그래비티(외부)로 분리한 점.
>
> **백업 모드**: 안티그래비티 불가·불신(silent drop 의심) 시 01 OCR·02-wiki까지 Claude Code가 `*_claude_*`로 직접 처리. → `백업플랜_ClaudeCode_단독.md`. (Gemini 누락 회피 + `integrity_check.py` 즉시 검증. 트리거: "백업 모드", "클로드 코드 단독")
...
3. **사용자가 v3.6 Phase 2 시범 운영 단계.** v3.5.2와 병행. 첫 책 1권 처리 후 사용자 검토 시간, 다음 책 진행 전 확인.
...
## v3.6 Phase 2 시범 운영 (현재)

- **첫 1~2주: 단권화 1챕터(50p)만 시범**
- 안티그래비티 위키화 본문 보존율(≥0.90)·책 유형 추정 신뢰도 90%+ 확인 후 다음 책
- 100장당 hallucination 측정
- 3장 초과 시 작업 중단

## 안전 규칙

- 01 OCR·02-wiki는 안티그래비티(Gemini 3.5 Flash) 측, 02-card·06은 Claude Opus 4.8(구독/Project)
...
다음은 카드화 단계 외부 API 자동화 시 사용 가능하도록 보존:
- `scripts/*.py` — 외부 API 자동화 (Anthropic API key 필요, 모델 = claude-opus-4-8)
- `.env`, `.env.example` — API key 설정
- `requirements.txt` — Python 의존성

현 운영 모드(v3.6 Phase 2)에서는 직접 미사용 (안티그래비티+Opus 분담 운영).

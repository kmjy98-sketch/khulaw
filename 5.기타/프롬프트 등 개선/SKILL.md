...
> Codex 운영 메모(2026-06-15): 이 SKILL은 레거시 Claude Code 패키지 설명이다. 현재 Codex 작업은 `.agent/workflows/card-wiki-pipeline.md`를 우선하고, 여기의 Claude Opus 호출·구독 의존 절차는 직접 실행 경로로 보지 않는다.
...
Step 0: pdf_split_100p.py (100p 사전 분할)
    │
    ▼
Step 1: 01_ocr.py (Gemini 3.1 Pro, Mode B default)
...
실행:
  ...
  2. 각 청크 01_ocr.py (Gemini 3.1 Pro, Mode B)
...
### Gemini 3.1 Pro (01번 OCR)
- 모델: `gemini-3.1-pro`
- 필수: `system_instruction` (prompts/01...md), `temperature` 1.0, `thinking_level` high, `media_resolution` high, `max_output_tokens` 64000
- 청크별 새 세션 (다중턴 PDF 회귀 버그 회피)
- 실패 시 재시도 3회
...
- Gemini 3.1 Pro: input $1.25/M, output $5/M

# 로스쿨 단권화·사례집 자동 카드화 파이프라인 (v3.6 Phase 1.2 — Claude Code 운영 모드)

Claude Code(이 세션) 안에서 PDF 직접 처리하는 프롬프트 묶음 패키지.
v3.5.2 수동 워크플로우(Claude Project·Gem 복사·붙여넣기) → v3.6 자동화 → v3.6 Phase 1.2 Claude Code 통합.

## v3.6 Phase 1.2 운영 방식 (2026-05-21)

**API 키 불필요**. Claude Code 구독으로 처리.
**외부 스크립트(scripts/*.py) 사용 안 함**. 추후 API key 발급 시 사용 가능하도록 보존.

| 항목 | 운영 |
|---|---|
| PDF 입력 | Claude Code 채팅에 직접 첨부 또는 inputs/raw_pdf/에 두기 |
| 프롬프트 적용 | Claude Code가 prompts/{단계}_claude_*.xml을 읽고 system context로 흡수 |
| 출력 | outputs/{01_ocr, 02_wiki, 02_cards, 06_cases}/ 자동 저장 |
| 검증 | Claude Code가 verification_protocol 체크리스트로 자가 검증 후 보고 |
...
## 환경 설정 (Claude Code 모드)

**불필요**. Claude Code 구독으로 모두 처리.

추후 외부 API 자동화 사용 시:
1. Anthropic Console에서 API key 발급
2. `.env.example` → `.env`로 복사 후 키 입력
3. `pip install -r requirements.txt`
4. `python scripts/01_ocr.py ...` 호출

## 비용 (Claude Code 모드)

- Claude Code 구독에 포함 (별도 API 비용 없음)
- 사용자의 Claude Code usage limit 내에서 처리

## 시범 운영 권장 순서 (v3.6 Phase 1.2)

1. **첫 명령**: SKILL.md 읽고 운영 모드 파악
2. **01번 단독 테스트**: 50p 분량 시범 PDF → 마킹·구조 인식 확인
3. **02-wiki 단독 테스트**: 01번 출력 → 책 유형 추정·속성 라벨·callout 변환 확인
4. **02-card 단독 테스트**: 위키 출력 → 노트 타입 자동 분기·표 분해 확인
5. **06b 단독 테스트**: 사례집 1문제 → 풀이 영역 식별·검증필요 태그 확인

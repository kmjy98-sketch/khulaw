#!/usr/bin/env python3
"""
PDF 100페이지 단위 분할 도구 (v3.4 운영용)

용도:
  한국 로스쿨 단권화 PDF(500~1500p)를 Claude API 안전 처리 단위로 분할.
  - Claude API PDF 한계: 32MB / 100페이지 / context 200K 토큰
  - (legacy: Gemini 3.1 Pro PDF 한계: 50MB / 1,000페이지 / 출력 64K 토큰)
  - 한국 법학 PDF는 한자·각주 밀도로 페이지당 토큰 무거움
  - 권장 분할 단위: 100페이지 (Claude 한계와 동일, Sonnet/Opus 안전 처리)

사용법:
  python pdf_split_100p.py <input.pdf> [--pages 100] [--out-dir ./chunks]
  
  예시:
    python pdf_split_100p.py 형법총론.pdf
    python pdf_split_100p.py 헌법쟁점정리.pdf --pages 80 --out-dir ./split

출력 파일명:
  {원본명}_p001-100.pdf, {원본명}_p101-200.pdf, ...
...
def split_pdf(
    ...
    for start in range(0, total_pages, pages_per_chunk):
        ...
        chunk_name = f"{input_path.stem}_p{start + 1:03d}-{end:03d}.pdf"
        ...
        if chunk_size_mb > GEMINI_PDF_LIMIT_MB:
            warnings.append(
                f"⚠ {chunk_size_mb:.1f}MB — Claude API 32MB 한계 초과. "
                f"--pages 값을 더 낮추세요."
            )
        if (end - start) > GEMINI_PDF_LIMIT_PAGES:
            warnings.append(
                f"⚠ {end - start}p — Claude API 100p 한계 초과."
            )
        ...
        print(
            ...
            f"p{start + 1:03d}-{end:03d} ({end - start}p, "
    ...
    log_data = {
        ...
        "pdf_library": PDF_LIB,
        "v3.4_anchor_template": (
            "각 청크를 Claude API에 업로드할 때 사용자 메시지에 "
            "다음 한 줄을 첨부:\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PDF 100페이지 단위 자동 분할 (v3.4 운영용)",
    ...
    if args.pages < 10 or args.pages > 500:
        sys.stderr.write(
            ...
            "  권장: 100 (Claude API 안전 단위, 한계와 동일)\n"
        )
        return 1

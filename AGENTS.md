...
7. 구글 포인터 파일(`.gdoc`/`.gsheet`/`.gslides`/`.gscript`)은 로컬 직접 읽기 불가 — Drive MCP로 열람한다. 루트 적치분은 `5.기타/구글문서/`로 집결한다(밀메뉴 관련·Gemini Gems·Colab Notebooks 제외, Opal 폐기 — 2026-06-11).
...
- 세션 시작·이전 작업 참고: `sync/wiki/_index.md`를 먼저 확인하고, 관련 wiki 아티클은 필요한 것만 읽는다.
- 메모리 정비/lint: `.agent/scripts/lint.py`
...
3. Spaced Repetition: `.agent/state/srs_log.json`에서 SM-2 간격 관리.
...
4. **#39 백링크 규칙**: 노트 본문 내 판례·조문·핵심 개념에 옵시디언 위키링크(`[[...]]`)를 사용한다.
   ...
   - 조문: `[[§XXX]]` 또는 `[[민법 제XXX조]]`. 같은 노트 내 하나의 형식으로 통일한다.
...
## Wiki 메모리 시스템 (#42-45)
1. 세션 시작 시 `sync/wiki/_index.md`가 있으면 먼저 확인한다. 관련 아티클이 있을 때만 개별 파일을 읽고, 전체 wiki를 무조건 로드하지 않는다.
...
4. wiki 아티클은 `sync/wiki/` 하위에만 저장한다. 오래된 아티클도 삭제하지 않고 `_trash/{YYYY-MM-DD}/`로 이동한다.

---
name: study-notes
description: "전사문, 교재, 기출문제를 기반으로 주요 내용 노트(핵심정리/개념정리/수업정리)를 자동 생성하는 스킬"
---
...
노트 생성 중 다음 상황이면 korean-law-mcp MCP 도구를 **자동 호출**한다:

1. **조문 원문 삽입**: 교재/전사문에서 조문 번호(민법 제○조 등)가 언급되었으나 원문이 없으면 `get_law_detail`로 원문 조회 → 요건 블록에 삽입
2. **판례 요지 보강**: 판례명/사건번호가 등장하되 판결요지가 없으면 `search_precedent_tool` → `get_precedent_detail`로 판결요지 한 줄 확인

...
description: 파일 분류 및 정리. 미분류 파일 검색, 분류 규칙 적용, 파일명 변경. "파일 정리", "분류", "미분류 파일", "새 파일 정리" 요청 시 사용.
---
...
- **신설 폴더**: `sync/_inbox/`(임시 격리), `sync/_발제/`(자유형 긴 제목), `sync/_meta/`(운영 메모), `sync/_답안지/{과목}/`(사례형 답안 통합 — 8과목).

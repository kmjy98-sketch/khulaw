...
def find_toc_for_file(file_path: Path, toc_files: List[Path]) -> Optional[Path]:
    """
    청크 파일에 가장 적합한 TOC 파일을 찾음
    규칙:
    1. 파일명에서 'Book Name' 부분 추출 (예: 1-01_민법_송영곤_기본민강_... -> 민법_송영곤_기본민강)
    2. TOC 파일들 중에서 'Book Name'을 포함하는 것 찾기
    3. 가장 긴 공통 문자열이나 유사도 기반 매칭
    """
    # 파일명 패턴: [번호]-[순서]_[과목]_[강사]_[책이름]_[파트]...
    # 예: 1-01_민법_송영곤_기본민강_권리주체(26)_...
    parts = strip_code_prefix(
        resolve_source_metadata(
            ROOT,
            file_path.stem,
            fallback_book_code=extract_book_code(file_path.stem),
    ...
    # 전략: 파일명의 앞부분(번호 제외)이 TOC 파일명에 포함되는지 확인
    # "민법_송영곤_기본민강" 이라는 시그니처를 찾기 위해 노력
    
    best_toc = None
    max_score = 0
    
    # 의미있는 키워드 추출 (번호, 페이지 등 제외)
    keywords = [p for p in parts if p and not re.match(r'^\d', p) and not re.search(r'p\d', p)]
    ...
    for toc in toc_files:
        ...
        # "목차" 파일은 보통 "1-00_...목차..." 형태
        # "책이름"이 일치하는 것이 가장 중요
        
        # 3. BookCode (1-xx) 기반 그룹핑 시도 (예: 1-00 과 1-01 은 같은 1- 시리즈)
        ...
        if file_code and file_code == toc_code:
            score += 2 # 같은 대분류 번호 가점
            
        if score > max_score and score >= 2: # 최소한의 일치 필요
            max_score = score
            best_toc = toc
...
def ingest_chunks(chunk_dir: Path, toc_path: Optional[Path] = None, batch_size: int = 10, reset: bool = False) -> dict:
    ...
    chunk_files = [f for f in chunk_dir.glob("*.md") if not is_toc_file(f) and f.stem != "range_toc"]
    ...
    # TOC 파일 자동 발견 및 로드
    toc_files = [f for f in chunk_dir.glob("*.md") if is_toc_file(f)]
    print(f"발견된 TOC 후보: {[t.name for t in toc_files]}")
    
    # TOC 캐시 (경로 -> 맵)
    toc_cache = {}
    
    if toc_path:
        # 강제 지정된 TOC가 있으면 그것만 로드
        toc_map = parse_toc_file(toc_path)
        toc_cache[toc_path] = toc_map
        print(f"지정된 TOC 사용: {toc_path.name}")
    ...
    all_pages = []
    chapters_found = set()
    toc_usage_count = {} # 어떤 TOC가 몇 번 쓰였는지
    
    for i, chunk_file in enumerate(chunk_files):
        try:
            current_toc_map = None
            
            if toc_path:
                current_toc_map = toc_cache[toc_path]
            elif toc_files:
                # 적절한 TOC 찾기
                matched_toc = find_toc_for_file(chunk_file, toc_files)
                if matched_toc:
                    if matched_toc not in toc_cache:
                        print(f"  [TOC 로드] {matched_toc.name}")
                        toc_cache[matched_toc] = parse_toc_file(matched_toc)
                    current_toc_map = toc_cache[matched_toc]
                    toc_usage_count[matched_toc.name] = toc_usage_count.get(matched_toc.name, 0) + 1
            
            pages = parse_chunk_file(chunk_file, current_toc_map)
            ...
            status_msg = f"[{i+1}/{len(chunk_files)}] {chunk_file.name}: {len(pages)}p"
            if current_toc_map:
                status_msg += " (TOC O)"
            print(status_msg)
    ...
    print(f"매핑된 챕터 수: {len(chapters_found)}")
    if toc_usage_count:
        print("TOC 사용 통계:")
        for name, count in toc_usage_count.items():
    ...
    meta = {
        ...
        "model": MODEL_NAME,
        "toc_mode": "auto" if not toc_path else str(toc_path),
